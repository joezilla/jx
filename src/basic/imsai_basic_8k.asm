...
;
        IFNDEF JXMON
JXMON   EQU     0               ;-1=JX MONITOR BIOS I/O
        ENDIF
        IFNDEF BASIC_STANDALONE
BASIC_STANDALONE EQU 0
        ENDIF
        IFNDEF JXMON_BEGPR
JXMON_BEGPR EQU 0               ;-1=SUPPRESS BEGPR (STANDALONE)
        ENDIF
;
; CPM EQUATES
;
...
INIT1:  IF      JXMON
        ...
        ENDIF
        LXI     H,STACK_TOP     ;MEMORY TOP = CONFIGURED STACK
        JMP     INIT3           ;SKIP SIO INIT + MEMORY DETECT
        ENDIF
        IF      NOT CPM AND NOT JXMON
...
NOTCO:  IF      JXMON
        CPI     7FH     ;TEST IF RUBOUT
        JZ      JXBS    ;DO DESTRUCTIVE BACKSPACE
        CPI     08H     ;TEST IF CTRL-H
        JNZ     NOTCH   ;BRIF NOT
JXBS:   DCX     H       ;POINT PRIOR
        MOV     A,M     ;LOAD PREV CHAR
        ORA     A       ;TEST IF BEGIN
        JZ      ECHO    ;BRIF IS (AT START OF LINE)
        MVI     A,08H   ;BACKSPACE
        CALL    TESTO
        MVI     A,' '   ;SPACE (ERASE CHAR)
        CALL    TESTO
        MVI     A,08H   ;BACKSPACE (MOVE BACK)
        CALL    TESTO
        JMP     TREAD   ;GET NEXT CHAR
        ENDIF
        IF      NOT JXMON
        CPI     7FH     ;TEST IF RUBOUT
        JNZ     NOTCH   ;BRIF NOT
